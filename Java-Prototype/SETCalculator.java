...
public class SETCalculator {
    ...
    public static void main(String[] args) {
        ...
        scan.nextLine();
        System.out.print("Input Y if all sailors are wearing EABs and N otherwise: ");  
        eabs = scan.nextLine();
        ...
        System.out.println("O2 Start Escape Time: " + oSET);
        coSET = coStartEscapeTime(canisters, fitSurv, unfitSurv, flood, coConc, vBreath, temp, remainingHr);
        System.out.println("CO2 Start Escape Time: " + coSET);
        if (eabs.equals("Y")){
            eabSET = eabStartEscapeTime(fitSurv, unfitSurv, finalP, vBreath, remainingHr);
            System.out.println("EAB Start Escape Time: " + eabSET);
        } else {
            System.out.println("EABs start escape time: N/A");
        }
        if (oSET < coSET) {
